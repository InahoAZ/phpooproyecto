<div class="box-principal">
	<h3 class="titulo">Agregar Clientes<hr></h3>
	<div class="panel panel-success">
		<div class="panel-heading">
			<h3 class="panel-title">Agregar un nuevo Cliente</h3>
		</div>
		<div class="panel-body">
			<div class="row">
				<div class="col-md-1"></div>
				<div class="col-md-10">
					<form id="form_agregar" class="form-horizontal" action="" method="POST" enctype="multipart/form-data">
						<div class="form-group">
							<label for="inputEmail" class="control-label">NOMBRE</label>
							<input class="form-control" name="nombre" id="nombre" type="text" >
							<label for="inputEmail" class="control-label">EDAD</label>
							<input class="form-control" name="edad"  id="documento" type="text" >
							<label for="inputEmail" class="control-label">CURSOS</label>
							<input class="form-control" name="curso" id="fnac"  type="text" >
							

						</div>
						<div class="form-group">
							<button type="submit" class="btn btn-success" >Registrar</button>
							<button type="reset" class="btn btn-warning">Borrar</button>
						</div>
					</form>
				</div>
				<div class="col-md-1"></div>
			</div>
		</div>
	</div>
</div>